...
| 资源 | 说明 |
|------|------|
| EC2 (m8g.medium) | 运行后端服务 + Nginx 反向代理 |
| RDS PostgreSQL | 数据库 |
...
5. **EC2 Key Pair** — 在 AWS 目标 Region（默认 `ap-northeast-1`）中创建好
...
2. 确保 Region 选择为 **ap-northeast-1**（Oregon）
...
cd infra
npm ci
npx cdk destroy -c stackName=SuperAgentTest --region ap-northeast-1
```
