...
# CloudWatch Agent
echo ">>> Resolving instance region via IMDSv2..."
IMDS_TOKEN=$(curl -s -X PUT "http://169.254.169.254/latest/api/token" -H "X-aws-ec2-metadata-token-ttl-seconds: 300" || true)
INSTANCE_REGION=$(curl -s -H "X-aws-ec2-metadata-token: $IMDS_TOKEN" http://169.254.169.254/latest/meta-data/placement/region || echo "ap-northeast-1")
...
{
  "agent": { "run_as_user": "root", "region": "__AWS_REGION__" },
...
# Substitute region placeholder with actual instance region
sed -i "s/__AWS_REGION__/$INSTANCE_REGION/g" /opt/aws/amazon-cloudwatch-agent/etc/amazon-cloudwatch-agent.json
